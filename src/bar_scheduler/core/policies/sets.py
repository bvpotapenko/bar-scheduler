...
def _base_sets(
    exercise: ExerciseDefinition, sparams: SessionTypeParams, latest_test_max: int | None
) -> int:
...
def _build_decayed_sets(
    num_sets: int, reps: int, curve: list[float], shape: _SetShape
) -> list[PlannedSet]:
...
class SetPrescriptor:
    ...
    def _target_reps(
        self, ctx: PrescriptionContext, sparams: SessionTypeParams, latest_test_max: int | None
    ) -> int:
    ...
    def _volume(
        self, ctx: PrescriptionContext, sparams: SessionTypeParams, signals: AdaptationSignals
    ) -> _Volume:
        ...
        adj_sets, adj_reps = self._autoreg.adjust(
            base,
            signals.ff_state,
            signals.history_sessions,
            sparams.sets_min,
        )
        return adj_sets, adj_reps, target_reps

    def _build(
        self, ctx: PrescriptionContext, sparams: SessionTypeParams, volume: _Volume, rest: int
    ) -> list[PlannedSet]:
