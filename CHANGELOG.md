...
### Added
- **Golden test suite** (`tests/golden/`) — 58 deterministic truth-snapshot tests across
  5 layers and 3 profiles (50 kg/160 cm, 80 kg/180 cm, 120 kg/190 cm):
  - Layer 1: profile creation, exercise/target/equipment initial-state assertions.
  - Layer 2: baseline history (10 sessions × 7 exercise-profile pairs) frozen against
    pre-calculated plan status, prescribed sets, and done-session metrics (`abs=0.01`).
  - Layer 3: improved TEST session → TM/prescription adaptation verified then reverted.
  - Layer 4: ~1.5× over-prescribed volume session → plan shifts upward, covers all 4
    exercise types (pull_up, dip, bss, incline_db_press).
  - Layer 5: 15-week overtraining/plateau/recovery simulation for dip and
    incline_db_press; `is_plateau`, `deload_recommended`, and readiness z-score asserted
    at checkpoints 7, 14, and 20.
- `regenerate.py` standalone script — re-bakes all GENERATED constants after any planner
  change; outputs are frozen in `constants_p1/p2/p3.py` Section B.
- `time-machine>=2.10` added to dev extras for deterministic `datetime.now()` freezing.

### Removed
- `explain_session()` API function and `explain_plan_entry()` core function removed.
  The explanation feature duplicated core logic, drifted out of sync whenever
  the planner changed, and was not used in practice.
- `session_trace_builder.py` and the `_SessionTrace` dataclass removed along with it.
- `_plan_core()` generator simplified: now yields `SessionPlan` directly instead
  of `(SessionPlan, _SessionTrace)` tuples.
