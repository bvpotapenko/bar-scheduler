...
| # | Feature | API function |
|---|---------|-------------|
| 6.1 | Training status: TM, latest TEST, trend slope, plateau flag, deload flag, fitness-fatigue state | `get_training_status` |
| 6.2 | 1-rep max estimate -- Epley, Brzycki, Lander, Lombardi, Blended formulas + recommended formula | `get_onerepmax_data` |
| 6.3 | Weekly rep volume | `get_volume_data` |
| 6.4 | Progress data for plotting: TEST session history + trajectory projections (BW reps, goal-weight reps, 1RM added kg) | `get_progress_data` |
| 6.5 | Overtraining severity assessment (level 0–3) | `get_overtraining_status` |
| 6.6 | Fitness-fatigue model: G(t) fitness, H(t) fatigue, readiness, readiness z-score | `get_training_status` -> `status["fitness"]` / `"fatigue"` / `"readiness_z_score"` |
